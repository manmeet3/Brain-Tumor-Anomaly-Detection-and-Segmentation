...
class TestBrats():
    ...
    # 1, load configure file
        ...
        self.self.config_net1 = config.get('network1', None)
        self.config_net2 = config.get('network2', None)
        self.config_net3 = config.get('network3', None)
        self.config_test = config['testing']  
        self.batch_size  = config_test.get('batch_size', 5)
        
    # 2.1, network for whole tumor
    if(self.config_net1):
        net_type1    = self.config_net1['net_type']
        net_name1    = self.config_net1['net_name']
        data_shape1  = self.config_net1['data_shape']
        label_shape1 = self.config_net1['label_shape']
        class_num1   = self.config_net1['class_num']
        ...
        net1.set_params(self.config_net1)
        predicty1 = net1(x1, is_training = True)
        proby1 = tf.nn.softmax(predicty1)
    else:
        self.config_net1ax = config['network1ax']
        self.config_net1sg = config['network1sg']
        self.config_net1cr = config['network1cr']
        
        # construct graph for 1st network axial
        net_type1ax    = self.config_net1ax['net_type']
        net_name1ax    = self.config_net1ax['net_name']
        data_shape1ax  = self.config_net1ax['data_shape']
        label_shape1ax = self.config_net1ax['label_shape']
        class_num1ax   = self.config_net1ax['class_num']
        ...
        net1ax.set_params(self.config_net1ax)
        predicty1ax = net1ax(x1ax, is_training = True)
        proby1ax = tf.nn.softmax(predicty1ax)

        # construct graph for 1st network sagittal
        net_type1sg    = self.config_net1sg['net_type']
        net_name1sg    = self.config_net1sg['net_name']
        data_shape1sg  = self.config_net1sg['data_shape']
        label_shape1sg = self.config_net1sg['label_shape']
        class_num1sg   = self.config_net1sg['class_num']
        ...
        net1sg.set_params(self.config_net1sg)
        predicty1sg = net1sg(x1sg, is_training = True)
        proby1sg = tf.nn.softmax(predicty1sg)
            
        # construct graph for 1st network corogal
        net_type1cr    = self.config_net1cr['net_type']
        net_name1cr    = self.config_net1cr['net_name']
        data_shape1cr  = self.config_net1cr['data_shape']
        label_shape1cr = self.config_net1cr['label_shape']
        class_num1cr   = self.config_net1cr['class_num']
        ...
        net1cr.set_params(self.config_net1cr)
