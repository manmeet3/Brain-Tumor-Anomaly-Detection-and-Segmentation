...
def infer():
    ...
    """
    global segmodel
    global segmodel_init
    if not segmodel_init:
    ...
    filepaths = req_json["filepaths"]
    for f_name in filepaths:
        if not os.path.isdir(f_name):
            ret_str = "Invalid path: %s", f_name
            return jsonify({"Return": "1", "Err": ret_str})

    segmodel.test(filepaths)
