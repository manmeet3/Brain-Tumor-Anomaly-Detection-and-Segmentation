...
def dat2canvas(data):
    ...
    print(f"dat2canvas: dn: {dn} dh: {dh} dw: {dw} dc: {dc}")
    ...
    for y in range(numd):
        for x in range(numd):
            try: tmp = data[x+(y*numd)]
                ...
                canvas[(y*dh):(y*dh)+512, (x*dw):(x*dw)+512, :] = tmp
    if(dc == 1):
...
def training_wgan(neuralnet, dataset, epochs, batch_size, normalize=True):
    ...
    for epoch in range(epochs):
        ...
        if(neuralnet.zdim == 2):
            ...
            print(f"training_wgan(): x_tr shape: {x_tr.shape}, z shape: {z_latents.shape}")
        ...
        while(True):
            print(f"training_wgan(): Training iteration {iteration}")
...
def training_izi(neuralnet, dataset, epochs, batch_size, normalize=True, iteration=0):
    ...
    #neuralnet.load_parameter(model='model_checker') # TEMP
    make_dir(path="training")
    ...
    for epoch in range(epochs):
        ...
        x_tr, y_tr, _ = dataset.next_train(batch_size=test_size, fix=True) # Initial batch
        z_tr = random_noise(x_tr.shape[0], neuralnet.zdim)
        print(f"training_izi(): x_tr shape: {x_tr.shape}, z shape: {z_tr.shape}")
...
def test(neuralnet, dataset, batch_size):
    ...
    outbound = loss_avg * 0.7 + (loss_std * 1)
