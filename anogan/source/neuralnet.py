...
class f_AnoGAN(object):
    ...
    def __init__(self, \
        ...
        self.path_ckpt = path
        print(f"h: {self.height}, w: {self.width}, c: {self.channel}")
    ...
    def step(self, x, z, iteration=0, training=False, phase=0):
        ...
        #print(f"step(): batch_size: {x.shape[0]}")
        summary_list = []
        if(training):
            try:
                if(phase == 0):
        ...
        #print(f"g_fake: {self.variables['g_fake']} loss_d: {self.losses['loss_d']} loss_g: {self.losses['loss_g']} loss_e: {self.losses['loss_e']}")
        #self.variables['g_fake'] = tf.Print(self.variables['g_fake'], [self.variables['g_fake']], message="=== g_fake: ")
        if(phase == 0):
            x_fake, loss_d, loss_g, loss_e = \
                self.sess.run([self.variables['g_fake'], self.losses['loss_d'], self.losses['loss_g'], self.losses['loss_e']], feed_dict=feed_te)
    ...
    def save_parameter(self, model='model_checker', epoch=-1):
        ...
        if(epoch >= 0 and epoch % 30 == 0): 
            print(f"Epoch: {epoch}. Adding metadata to summary writer")
            self.summary_writer.add_run_metadata(self.run_metadata, 'epoch-%d' % epoch) #ms: added epoch % 30 == 0 to save when this is written
    ...
    def __init_session(self, path):
        ...
        try:
            ...
            self.run_options = tf.compat.v1.RunOptions(trace_level=tf.compat.v1.RunOptions.SOFTWARE_TRACE) #ms: original: FULL_TRACE
            self.run_metadata = tf.compat.v1.RunMetadata()
        except: pass

    def loss_l2(self, a, b, reduce=None):
        #print(f"loss_l2: a: {a} b:{b}")
    ...
    def __encoder(self, x, ksize=3, outdim=1, reuse=False, \
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            rs = tf.compat.v1.reshape(x, shape=[self.batch_size, int(128*128*64)], \
                name="%s_rs" %(name))
    ...
    def __decoder(self, z, ksize=3, reuse=False, \
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            fc1 = self.layer.fully_connected(x=z, c_out=128*128*64, \
                batch_norm=True, training=self.training, \
                activation=activation, name="%s_fc1" %(name), verbose=verbose)
            #print(f"=================================decoder: fc1 {fc1}")
            rs = tf.compat.v1.reshape(fc1, shape=[self.batch_size, 128, 128, 64], \
                name="%s_rs" %(name))
